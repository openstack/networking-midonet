...
from neutron.tests.unit.db.loadbalancer import test_db_loadbalancer as test_lb
...
class LoadbalancerTestCase(test_lb.LoadBalancerTestMixin,
    ...
    def setUp(self):
        ...
        super(LoadbalancerTestCase, self).setUp(ext_mgr=ext_mgr)
        ...
        test_lb._subnet_id = self._subnet_id
    ...
    def test_create_multiple_pools_same_subnet(self):
        name1 = "pool1"
        name2 = "pool2"
        keys1 = [('name', name1),
                 ('subnet_id', test_lb._subnet_id),
        ...
        keys2 = [('name', name2),
                 ('subnet_id', test_lb._subnet_id),
                 ('tenant_id', self._tenant_id)]
        with self.pool(name=name1) as pool1:
            with self.pool(name=name2) as pool2:
                for k, v in keys1:
                    self.assertEqual(pool1['pool'][k], v)
                for k, v in keys2:
                    self.assertEqual(pool2['pool'][k], v)
    ...
    def test_create_pool_with_no_router(self):
        ...
        with self.subnet(cidr='10.0.1.0/24') as sub:
